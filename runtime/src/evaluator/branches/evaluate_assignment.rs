...
mod tests {
    ...
    fn should_assign_new_value() {
        ...
        // TODO: Add utility function for creating an environment, scope and
        // evaluator for testing, since this will be a common setup.
        let environment = Environment::new();
        let mut scope = Scope::new();
        scope
            .add_variable(&variable_name, Value::Number(variable_value), true)
            .unwrap();
        let evaluator = &Evaluator::new(std::rc::Rc::new(
            std::cell::RefCell::new(environment),
        ));
        ...
        let result = evaluate_assignment(
            Assignment::new(
                ...
                Box::new(Expression::Literal(Literal::Decimal(
                    DecimalLiteral::new(variable_value_to_assign),
                ))),
            ),
            evaluator,
            &mut scope,
        ...
        let Some(ScopeItem::Variable { value, .. }) =
            scope.lookup_current_scope(&variable_name)
        else {
            panic!(
                "Expected variable to have been added to scope when preparing the test."
            )
        };
