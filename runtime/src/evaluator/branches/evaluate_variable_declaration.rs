...
/// Evaluate a variable declaration by evaluating the initial value (if it
/// exists) and adding the variable to the scope. Returns `Value::Nil` if the
/// variable declaration was successful, or an error if it was not.
pub fn evaluate_variable_declaration(
    ...
    let initial_value: Option<Value> = match variable_declaration.initial_value
    {
        Some(initial) => match evaluator.evaluate_expression(*initial, scope) {
            ...
            EvaluationResult::Throw(err) => {
                return EvaluationResult::Throw(err);
            }
    ...
    // TODO: Add utility function for adding a variable to the scope and
    ...
    match scope
        ...
        .map_err(|error| {
            Value::String(format!("Error declaring variable: {:?}", error))
        })
        .map(|_| Value::Nil)
    {
