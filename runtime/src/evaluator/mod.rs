...
impl Evaluator {
    ...
    /// Run all of the expressions in the program and return the value of the
    /// last expression.
    pub fn evaluate_program(&mut self, program: Program) -> EvaluationResult {
        let mut scope = Scope::with_parent(Box::new(
            // TODO: This is a bit of a hack to get around the fact that the
            // scope needs to be mutable, but the environment is
            // not. This should be refactored.
            self.environment.take().global_scope,
        ));

        evaluate_expressions_and_return_last_value(
            program.body.items,
            self,
            &mut scope,
        )
